...
class DocsSidebarNavItem extends React.Component {
  ...
  isActive() {
    ...
    const isActive =
      getNormalizedPath(href) === getNormalizedPath(location.pathname)
  ...
  isActiveRoot() {
    ...
    const isActive = node =>
      getNormalizedPath(href(node)) === getNormalizedPath(location.pathname)
    const hasActiveChild = node =>
      !node.children
        ? false
        : node.children.some(node => isActive(node) || hasActiveChild(node))
  ...
  componentDidUpdate(prevProps) {
    if (
      getNormalizedPath(this.props.location.pathname) !==
      getNormalizedPath(prevProps.location.pathname)
    ) {
  ...
  render() {
    ...
    return (
      <li className="DocsSidebar--nav-item" key={node.id} {...props}>
        {this.showChildren() && (
          <button
